...
// This is an special release to rollback to 1.9.1
// We rewrote the plugin to use the API provided by ChatGPT for providing richer features
// However, the history API has been temporarily disabled by OpenAI
// We will continue to work on this plugin and release a new version when the API is available again
...
(function() {
  ...
  function getConversation() {
    ...
    document.querySelectorAll("main .group").forEach((item) => {
      ...
      if (paragraphNodes.length === 0)
        paragraphNodes = Array.from(item.querySelectorAll(".w-full .whitespace-pre-wrap"));
      if (paragraphNodes.length === 0)
        return;
  ...
  function orderedListToMarkdown(node, lineMapper) {
    ...
    return node.items.map((item, index) => `${start + index}. ${item.map((line) => lineMapper(line)).join("\n")}`).join("\n");
  }
  function unorderedListToMarkdown(node, lineMapper) {
    return node.items.map((item) => `- ${item.map((line) => lineMapper(line)).join("\n")}`).join("\n");
  }
    ...
    return line.map((node) => {
      ...
      switch (nodeType) {
        ...
        case "ordered-list-item": {
          ...
          const content2 = node.items.map((item) => `<li>${item.map((line2) => lineToHtml(line2)).join("\n")}</li>`).join("");
          return `<ol${start}>${content2}</ol>`;
        }
        case "unordered-list-item": {
          const content2 = node.items.map((item) => `<li>${item.map((line2) => lineToHtml(line2)).join("\n")}</li>`).join("");
          return `<ul>${content2}</ul>`;
        }
        case "code":
